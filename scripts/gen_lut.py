...
def build_table(precision: int) -> List[str]:
    ...
    for i in range(256):
        ...
        if i == 0:
            ...
            out.append(
                fmt.format(lin).rstrip("0").rstrip(".")
                if precision > 0
                else fmt.format(lin)
            )
    return out
...
def emit_zig(
    table: List[str], elem_type: str, header: str, trailing: bool, minimize: bool
) -> str:
...
def main(argv: List[str]) -> int:
    ...
    ap.add_argument(
        "--precision", type=int, default=10, help="Digits after decimal (default: 10)."
    )
    ap.add_argument(
        "--type",
        dest="elem_type",
        default="f32",
        help="Element type: zig f32 / C float, etc.",
    )
    ap.add_argument(
        "--header", default="SRGB_LUT", help="Symbol name / left-hand side."
    )
    ap.add_argument(
        "--no-trailing",
        action="store_true",
        help="Omit trailing comma on last element.",
    )
    ...
    if args.lang == "zig":
        sys.stdout.write(
            emit_zig(table, args.elem_type, args.header, trailing, args.minimize)
        )
    else:
